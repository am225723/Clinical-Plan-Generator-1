...
"[project]/pages/api/dashboard/appointments.ts [api] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function handler(req, res) {
    ...
    try {
        ...
        const { data: appointmentsData, error } = await supabase.from('appointments').select('id, patient_name, appointment_type, location, scheduled_time, status, has_flag, notes_pending, attached_note, document_id, external_calendar_id').eq('doctor_id', user.id).gte('scheduled_time', nowIso).order('scheduled_time', {
            ascending: true
        }).limit(25);
        ...
        const appointments = (appointmentsData || []).map((appointment)=>{
            ...
            const status = appointment.status === 'completed' ? 'completed' : appointment.status === 'in_progress' ? 'in_progress' : 'pending';
            return {
                id: appointment.id,
                time,
                period,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__9add2cb6._.js.map